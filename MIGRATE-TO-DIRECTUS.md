...
## ⚑ OWNER DECISIONS (2026-06-11) — these are FINAL and override the "PROPOSED/recommended/UNKNOWN" notes below

1. **Email routing IS in v1 — it is core to the system.** Do NOT defer it to a later phase. The Outlook
   email-ingest → AI-router → ClickUp-sync pipeline, the `emailMessage` / `ignoreRule` / `aiModelConfig`
   objects, and the Fireflies meeting-transcription flow must all be reproduced on the Directus backend as
   part of v1. (Directus has Flows + the ability to run a companion worker/Cloudflare Worker; plan the
   router as a Directus-triggered worker, not a deferral.) Treat §4.2 and §5's "defer to Phase 2" wording as
   **superseded** — these become `crm_email_message`, `crm_ignore_rule`, `crm_ai_model_config` v1 collections
   plus the routing worker.
2. **Reuse the PIM tables — do NOT create `crm_company` / `crm_contact` / `crm_factory`.** The CRM "company"
   IS the PIM **`retailer`** collection, the CRM "contact" IS the PIM **`buyer`** collection, and the CRM
   "factory" IS the PIM **`factory`** collection. Extend those existing PIM collections with the CRM-specific
   fields (from §4.1/§4.2/§5.1) rather than making parallel tables. All CRM relations that §5 points at
   `crm_company`/`crm_contact`/`crm_factory` instead point at `retailer`/`buyer`/`factory`. This is Option 1
   in §5.2 — chosen deliberately for a single source of truth across PIM+CRM. (Add missing columns to the PIM
   collections additively; never drop/disturb the 651 projects + 16,534 products already in the shared DB.)

The remaining new CRM collections (still `crm_`-prefixed) are: `crm_opportunity`, `crm_department`,
`crm_meeting_note`, `crm_meeting_note_attendee`, `crm_licensor_approval_thread`, `crm_email_message`,
`crm_ignore_rule`, `crm_ai_model_config`, plus `crm_note` / `crm_task` if used.
...
> **Migration scope (DECIDED — see ⚑ OWNER DECISIONS):** the email-routing machinery (`emailMessage`,
> `ignoreRule`, `aiModelConfig`, the Outlook-ingest/router/ClickUp-sync crons, Fireflies webhook) is
> **CORE and ships in v1** — it is the most fork-specific part of Twenty and the hardest to reproduce, but
> it is not optional. Plan it as a Directus-triggered companion worker (Directus Flows for the triggers +
> a Node/Cloudflare worker for the multi-step router/AI/ClickUp-sync, since Directus Flows alone don't
> express Twenty's pipeline). Reproduce `emailMessage`/`ignoreRule`/`aiModelConfig` as live v1 collections,
> not read-only history. Extract the exact router logic from the Twenty fork's email-routing crons/services
> before rebuilding (see §6.2 — do not guess the routing rules or AI prompts).
...
- **CRM company ↔ PIM retailer/buyer.** PIM already has `retailer` ("A store / account (future CRM
  ...
  **DECIDED (owner, 2026-06-11): Option 1 — reuse the PIM tables.** CRM company = PIM `retailer`,
  CRM contact = PIM `buyer`, CRM factory = PIM `factory`. Add the CRM-specific fields onto those
  existing collections additively (never drop columns or disturb existing rows). Do NOT create
  `crm_company`/`crm_contact`/`crm_factory`. Every §5.1 relation that named those points at
  `retailer`/`buyer`/`factory` instead.
