  ...
  async handleWebhook(
    ...
    return this.globalWorkspaceOrmManager.executeInWorkspaceContext(
      async () => {
        ...
        // ── Route to program ───────────────────────────────────────────
        const routing = await this.emailRouterService.routeEmail(workspaceId, {
        ...
        if (!routing.companyId) {
          ...
          const externalParticipants = (transcript.participants ?? []).filter(
            (e) => !e.toLowerCase().endsWith('@popcre.com'),
          );
        ...
        const internalParticipants = (transcript.participants ?? []).filter(
          (e) => e.toLowerCase().endsWith('@popcre.com'),
        );
        ...
        const saved = (await meetingNoteRepo.save(
          meetingNote,
        )) as unknown as MeetingNoteWorkspaceEntity;
