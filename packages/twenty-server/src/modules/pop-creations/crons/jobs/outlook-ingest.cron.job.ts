...
export class OutlookIngestCronJob {
  ...
  private async processWorkspace(workspaceId: string): Promise<void> {
    ...
    const emails = await this.fetchRecentEmails(accessToken, mailbox, since);

    if (emails.length === 0) {
      this.logger.debug(`No new emails found for ${mailbox} since ${since}`);
  ...
  private async processEmail(
    ...
    if (isGated) {
      const hasExternal = allAddresses.some(
        (addr) => !addr.toLowerCase().endsWith(`@${INTERNAL_DOMAIN}`),
      );
    ...
    const ccAddresses = graphEmail.ccRecipients
      .map((r) => r.emailAddress.address)
      .join(', ');
    const recipients = [toAddresses, ccAddresses].filter(Boolean).join(', ');
    ...
    const { soNumbers: detectedSoNumbers, poNumbers: detectedPoNumbers } =
      this.soExtractorService.extractSeparate(bodyForExtraction);
    ...
    // ── AI summary update ──────────────────────────────────────────────────
    // When an email is successfully routed to a specific opportunity,
    // update that opportunity's AI-generated summary in the background.
    if (routingResult.programId) {
      this.opportunitySummaryService
        .updateSummary(workspaceId, routingResult.programId, {
          subject: graphEmail.subject ?? '(no subject)',
          bodyPreview: graphEmail.bodyPreview ?? '',
          sender: graphEmail.from?.emailAddress?.address ?? '',
        })
        .catch((err) =>
          this.logger.warn(
            `Background summary update failed for ${routingResult.programId}: ${err}`,
          ),
        );
    }
  }
  ...
  private async resolveWorkspaceMember(
    ...
    try {
      ...
      const match = members.find(
        (m: any) => m.userEmail?.toLowerCase() === mailboxEmail.toLowerCase(),
      );
