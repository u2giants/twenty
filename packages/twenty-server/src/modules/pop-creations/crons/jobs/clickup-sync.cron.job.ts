...
export class ClickUpSyncCronJob {
  ...
  async handle(): Promise<void> {
    ...
    if (!clickupToken) {
      this.logger.warn(
        'Missing CLICKUP_API_TOKEN. Skipping ClickUp sync.',
      );
  ...
  private async autoLinkByPoNumber(
    ...
    for (const listId of ALL_LIST_IDS) {
      try {
        const tasks = await this.fetchClickUpListTasks(
          listId,
          clickupToken,
        );

        for (const task of tasks) {
          const sasPo = this.extractCustomFieldValue(
            task,
            'SAS-PO',
          );
