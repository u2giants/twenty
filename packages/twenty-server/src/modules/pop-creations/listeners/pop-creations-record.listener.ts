...
export class PopCreationsRecordListener {
  ...
  async handlePersonCreated(
    ...
    await this.globalWorkspaceOrmManager.executeInWorkspaceContext(async () => {
      ...
      for (const event of events) {
        ...
        // --- Auto-set scope based on jobTitle ---
        if (!person.scope && person.jobTitle) {
          if (BUYER_CONTACT_TYPES.includes(person.jobTitle)) {
            updates.scope = 'DEPARTMENT';
          } else if (COMPANY_WIDE_CONTACT_TYPES.includes(person.jobTitle)) {
            updates.scope = 'COMPANY_WIDE';
          } else if (
            person.jobTitle === 'OTHER' ||
            person.jobTitle === 'FORMER_CONTACT'
          ) {
  ...
  // ─── 4b. Department soft-deleted → clear departmentId from linked people ──

  @OnDatabaseBatchEvent('department', DatabaseEventAction.UPDATED)
  async handleDepartmentSoftDeleted(
    payload: WorkspaceEventBatch<
      ObjectRecordUpdateEvent<DepartmentWorkspaceEntity>
    >,
  ) {
    const { workspaceId, events } = payload;

    const softDeletedIds = events
      .filter(
        (e) => !e.properties.before.deletedAt && e.properties.after.deletedAt,
      )
      .map((e) => e.properties.after.id);
    ...
    await this.globalWorkspaceOrmManager.executeInWorkspaceContext(async () => {
      ...
      for (const deptId of softDeletedIds) {
        const result = await personRepo.update(
          { departmentId: deptId } as any,
          { departmentId: null } as any,
        );

        this.logger.log(
          `Department ${deptId} soft-deleted: cleared departmentId from ${result.affected ?? 0} people`,
        );
      }
    }, authContext);
  }
      ...
      for (const event of events) {
        ...
        switch (after.stage) {
          ...
          case 'IN_PRODUCTION':
            ...
            if (after.sampleRequired) {
              tasksToCreate.push('Request sample with production PO');
            }
            break;
        }
    ...
    await this.globalWorkspaceOrmManager.executeInWorkspaceContext(async () => {
      for (const event of events) {
        ...
        if (taskTitle && after.programId) {
          await this.createTasksForOpportunity(workspaceId, after.programId, [
            taskTitle,
          ]);
    ...
    await this.globalWorkspaceOrmManager.executeInWorkspaceContext(async () => {
      for (const event of events) {
        ...
        while (hasMore) {
          ...
          const matchingIds: string[] = [];
          for (const email of emails) {
            const emailNormalized = this.normalizeSubject(email.subject);
            if (emailNormalized.toLowerCase() === normalized.toLowerCase()) {
              matchingIds.push(email.id);
            }
          }
